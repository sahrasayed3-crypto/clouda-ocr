...
async function renderDatasetDetail(datasetId) {
  ...
  app.append(panel("Lineage", [lineage(data.lineage || ["Raw Source", "Training Dataset"])]));
...
async function renderQuality() {
  ...
  const select = node("select", { "aria-label": "Dataset" }); datasets.forEach((item) => select.append(node("option", { value: item.dataset_id, text: item.identity }))); const output = node("input", { value: "clean-derived", "aria-label": "Derived dataset label" }); const result = node("div");
...
async function renderPlanner() {
  ...
  function selectAdapter() { const selected = models.find((item) => item.adapter_id === fields.adapter_type.value); if (!selected) return; fields.model_id.value = selected.upstream_repository; clear(fields.precision); selected.supported_precision.forEach((item) => fields.precision.append(node("option", { value: item, text: item }))); }
...
async function renderRunDetail(runId) {
  ...
  app.append(panel("Lineage", [lineage([run.dataset_id || "Dataset", run.config_hash || "Plan", run.run_id || "Run", "Checkpoint", "Resume"])]));
  app.append(panel("Checkpoints", [(checkpointData.checkpoints || []).length ? table(["Checkpoint", "Step", "Created", "Integrity", "Resume state"], checkpointData.checkpoints.map((item) => [item.checkpoint_id, item.step, item.timestamp, badge(item.integrity), badge(item.resume_state || "VALIDATION REQUIRED")])) : node("div", { class: "state", text: "No checkpoints are recorded." })]));
  ...
  app.append(panel("Resume", [button("Validate Resume", checkResume, { class: "secondary" }), button("Start Real Training", () => {}, { disabled: true, title: "No compatible GPU is currently available" }), node("p", { class: "muted", text: "No compatible GPU is currently available. Resume execution remains disabled; compatibility validation is available." }), resumeResult]));
}
