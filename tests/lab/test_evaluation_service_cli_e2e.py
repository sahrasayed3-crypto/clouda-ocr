...
def _synthetic_samples() -> list[OCRSample]:
    return [
        OCRSample(
            ...
            metadata={"profile": profile, "split": "test",
                      "document_type": "book", "source": "synthetic"},
        )
...
class TestEvaluationService:
    ...
    def test_compare_models(self):
        service = EvaluationService()
        baseline = [SampleMetrics(sample_id="p-01", cer=0.40, wer=0.5),
                    SampleMetrics(sample_id="p-02", cer=0.10, wer=0.2)]
        candidate = [SampleMetrics(sample_id="p-01", cer=0.15, wer=0.25),
        ...
        assert report2.counts["regressed"] == 1  # p-02 now crosses the tighter threshold
    ...
    def test_buckets_hard_and_recommend(self):
        ...
        assert len(hard) == 2
        recommendation = service.recommend_next(rows, batch_size=3, seed=1, history=("p-01",))
    ...
    def test_exports(self, tmp_path: Path):
        ...
        baseline = [SampleMetrics(sample_id=s.sample_id, cer=0.5, wer=0.6) for s in samples]
        candidate = [SampleMetrics(sample_id=s.sample_id, cer=analysis.cer, wer=analysis.wer)
                     for s in samples]
        failure = service.compare_models(baseline, candidate,
        ...
        assert json.loads(out_c.read_text(encoding="utf-8"))["summary"]["common_samples"] == 6


class TestIOLoaders:
    def test_load_samples_jsonl(self, tmp_path: Path):
        records = [
            {"sample_id": "x1", "reference_text": "مرحبا", "prediction_text": "مراحبا",
             "model_id": "m", "profile": "clean", "severity": "low"},
...
class TestCLI:
    def test_analysis_batch_json(self, tmp_path: Path, capsys):
        ...
        path.write_text(
            "\n".join(json.dumps({
                "sample_id": s.sample_id, "reference_text": s.ground_truth,
                "prediction_text": s.prediction, "model_id": s.model_id,
                **s.metadata}, ensure_ascii=False)
                for s in samples),
            encoding="utf-8",
        )
        out = tmp_path / "batch.json"
        exit_code = cli_main(["analysis-batch", "--input", str(path), "--output", str(out)])
        assert exit_code == 0
        payload = json.loads(out.read_text(encoding="utf-8"))
        assert payload["total_samples"] == 6

    def test_dataset_select_and_validate(self, tmp_path: Path, capsys):
        rows = [{"_schema_version": "clouda.pretraining.manifest.v1", "_row_count": 3,
                 "dataset_role": "training"}]
        rows.extend(
            {"sample_id": f"s-{i}", "target_split": "train", "source_id": "src",
             "source_license": "Apache-2.0", "cer": i / 10}
            for i in range(3)
        )
        ...
        exit_code = cli_main([
            "dataset-select", "--manifest", str(manifest), "--output", str(derived),
            "--cer-min", "0.1", "--seed", "3",
        ])
        assert exit_code == 0
    ...
    def test_hard_examples_cli(self, tmp_path: Path):
        rows = [{"sample_id": f"s{i}", "cer": i / 10, "wer": i / 10, "ncer": i / 20}
                for i in range(5)]
        path = tmp_path / "rows.json"
        path.write_text(json.dumps(rows), encoding="utf-8")
        out = tmp_path / "hard.json"
        exit_code = cli_main(["dataset-hard-examples", "--input", str(path),
                              "--top-n", "2", "--output", str(out)])
        assert exit_code == 0
        payload = json.loads(out.read_text(encoding="utf-8"))
    ...
    def test_recommend_next_cli(self, tmp_path: Path):
        rows = [
            {"sample_id": f"a{i}", "cer": 0.5, "wer": 0.6, "ncer": 0.4,
             "profile": "clean", "error_type_counts": {"whitespace": 1},
             "model_id": "m"} for i in range(3)
        ] + [
            {"sample_id": f"b{i}", "cer": 0.4, "wer": 0.5, "ncer": 0.3,
             "profile": "rough", "error_type_counts": {"diacritic": 1},
             "model_id": "m"} for i in range(3)
        ]
        path = tmp_path / "rows.json"
        path.write_text(json.dumps(rows), encoding="utf-8")
        out = tmp_path / "rec.json"
        exit_code = cli_main(["dataset-recommend-next", "--input", str(path),
                              "--strategy", "balanced_hard", "--batch-size", "4",
                              "--output", str(out)])
        assert exit_code == 0
        payload = json.loads(out.read_text(encoding="utf-8"))
...
class TestEndToEnd:
    def test_full_backend_flow(self, tmp_path: Path):
        ...
        baseline_metrics = [
            SampleMetrics(sample_id=s.sample_id,
                          cer=a.cer + 0.10 if a.cer < 0.3 else a.cer + 0.20,
                          wer=a.wer + 0.10, ncer=a.normalized_cer + 0.05,
                          error_types=a.error_type_counts)
            for s, a in zip(samples, analyses)
        ]
        candidate_metrics = [
            SampleMetrics(sample_id=s.sample_id, cer=a.cer, wer=a.wer,
                          ncer=a.normalized_cer, error_types=a.error_type_counts)
            for s, a in zip(samples, analyses)
        ]
        ...
        failure = service.compare_models(
            ...
            baseline_id="run-000", candidate_id="run-001",
        )
        assert failure.counts["improved"] + failure.counts["recovered"] >= len(samples) - 2
        assert failure.counts["regressed"] == 0
        assert failure.counts["newly_failed"] == 0

        # 4. analysis rows → hard examples → recommendation
        rows = [
            {"sample_id": s.sample_id, "cer": a.cer, "wer": a.wer,
             "ncer": a.normalized_cer, "error_type_counts": a.error_type_counts,
             "profile": s.metadata["profile"], "distortion": s.metadata["profile"],
             "model_id": s.model_id}
            for s, a in zip(samples, analyses)
        ]
        hard = service.hard_examples(rows, top_n=3)
        recommendation = service.recommend_next(rows, strategy="balanced_hard",
                                                batch_size=4, seed=42,
                                                history=tuple(h.sample_id for h in hard[:1]))
        assert all(h.sample_id not in [x.sample_id for x in recommendation.selections]
                   for h in recommendation.selections[:0])  # structure sanity
        assert len(recommendation.selections) == 4

        # 5. derived training manifest from selection
        manifest_rows = [
            {"_schema_version": "clouda.pretraining.manifest.v1", "_row_count": len(rows),
             "dataset_role": "training", "dataset_id": "synthetic-fixture",
             "dataset_version": "v1"}
        ]
        manifest_rows.extend(
            {"sample_id": row["sample_id"], "target_split": "train",
             "source_id": "synthetic", "source_license": "Apache-2.0"}
            for row in rows
        )
        ...
        artifacts = [
            export_json({"flow": "e2e", "samples": len(samples)}, tmp_path / "e2e_summary.json")
        ]
