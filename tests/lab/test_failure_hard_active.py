...
def _m(
    sample_id: str, cer: float, wer: float = 0.0, ncer: float = 0.0, **kw
) -> SampleMetrics:
...
class TestFailureClassification:
    ...
    def test_duplicate_sample_ids_rejected(self):
        with pytest.raises(ValueError, match="Duplicate"):
            compare_failure(
                [_m("s1", 0.1), _m("s1", 0.2)], [], baseline_id="a", candidate_id="b"
            )

    def test_report_to_dict(self):
        report = compare_failure(
            [_m("s1", 0.4)], [_m("s1", 0.1)], baseline_id="base", candidate_id="cand"
        )
...
class TestFailureBuckets:
    ...
    def test_deletion_insertion_buckets(self):
        deletions = bucket_sample(
            error_type_counts={"missing_word": 3, "whitespace": 1}
        )
...
class TestActiveLearning:
        ...
        return [
            {
                ...
                "error_type_counts": {"character_substitution": 2},
                "model_id": "m1",
            }
            for i in range(6)
        ] + [
            {
                ...
                "wer": 0.6,
                "ncer": 0.4,
                "profile": "phone_photo_medium",
                "distortion": "skew",
                "error_type_counts": {"whitespace": 2},
                "model_id": "m1",
            }
            for i in range(6)
        ]
    ...
    def test_hardest_only(self):
        rec = recommend_next_batch(
            self._rows(), strategy="hardest_only", batch_size=4, seed=1
        )
    ...
    def test_balanced_hard_respects_quotas(self):
        rec = recommend_next_batch(
            self._rows(), strategy="balanced_hard", batch_size=4, seed=1
        )
    ...
    def test_diversity_first_covers_categories(self):
        rec = recommend_next_batch(
            self._rows(), strategy="diversity_first", batch_size=6, seed=1
        )
        ids = [s.sample_id for s in rec.selections]
    ...
    def test_deterministic_random(self):
        first = recommend_next_batch(
            self._rows(), strategy="deterministic_random", batch_size=5, seed=9
        )
        second = recommend_next_batch(
            self._rows(), strategy="deterministic_random", batch_size=5, seed=9
        )
        other = recommend_next_batch(
            self._rows(), strategy="deterministic_random", batch_size=5, seed=10
        )
        assert [s.sample_id for s in first.selections] == [
            s.sample_id for s in second.selections
        ]
        assert [s.sample_id for s in first.selections] != [
            s.sample_id for s in other.selections
        ]

    def test_mixed_curriculum(self):
        rec = recommend_next_batch(
            self._rows(), strategy="mixed_curriculum", batch_size=6, seed=1
        )
