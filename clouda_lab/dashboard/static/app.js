...
const state = { actionToken: null, lastPlanId: null, requestController: null, taskPoll: null };
...
const pages = {
  ...
  "model-catalog": ["Model Catalog", "Published benchmark models and managed local assets"],
  ...
  "benchmark-workspace": ["Benchmark Workspace", "Deterministic plans over published evidence"],
  "storage": ["Storage", "Managed roots, capacity, and network policy"],
...
async function confirmPlan(plan, endpoint, label) { const summary = `${label}\n\nItem: ${plan.item || plan.target_id}\nEstimated bytes: ${plan.estimated_bytes ?? plan.bytes ?? "unknown"}\nDestination: ${plan.destination || "managed local storage"}`; if (!window.confirm(summary)) return null; return action(() => api(endpoint, { method: "POST", body: { plan_id: plan.plan_id, confirmation: plan.confirmation_token } }), label); }
...
async function renderDownloads() {
  const [sourceData, downloadData, importData] = await Promise.all([api("/sources"), api("/downloads"), api("/imports")]); const sources = sourceData.sources || []; const downloads = downloadData.downloads || []; const imports = importData.imports || []; const result = node("div");
  async function begin(source) { const plan = await action(() => api(`/sources/${encodeURIComponent(source.source_id)}/download-plan`, { method: "POST", body: {} }), "Preparing the canonical sample download plan…"); clear(result); result.append(panel("Download plan", [kv({ "Source": plan.item, "Provider": plan.provider, "License": plan.license, "Estimated bytes": plan.estimated_bytes, "Maximum bytes": plan.max_bytes, "Destination": plan.destination }), details("Expected files and restrictions", { expected_files: plan.expected_files, usage_restrictions: plan.usage_restrictions }), button("Confirm Download", async () => { const task = await confirmPlan(plan, "/dataset-downloads", "Confirm explicit dataset sample download"); if (task) location.hash = "#/tasks"; })])); }
  async function verify(source) { const task = await action(() => api(`/sources/${encodeURIComponent(source.source_id)}/verify`, { method: "POST", body: {} }), "Starting canonical dataset verification…"); if (task) location.hash = "#/tasks"; }
  async function remove(source) { const plan = await action(() => api(`/sources/${encodeURIComponent(source.source_id)}/removal-plan`, { method: "POST", body: {} }), "Preparing recoverable dataset removal…"); const task = await confirmPlan(plan, "/dataset-removals", "Remove managed dataset copy to recoverable trash"); if (task) location.hash = "#/tasks"; }
  function sourceActions(source) { const local = source.local_state === "DOWNLOADED"; return node("div", { class: "toolbar compact" }, [button("Review Download", () => begin(source), { disabled: !source.download?.available || local, title: local ? "A managed copy is already present" : (source.download?.reason || "Create a bounded sample plan") }), button("Verify", () => verify(source), { class: "secondary", disabled: !local, title: local ? "Verify canonical manifest and files" : "No managed copy is present" }), button("Remove", () => remove(source), { class: "secondary", disabled: !local, title: local ? "Move the managed copy to recoverable trash" : "No managed copy is present" })]); }
  async function validateImport(importId) { const data = await action(() => api(`/imports/${encodeURIComponent(importId)}/validate`, { method: "POST", body: {} }), "Validating the managed source manifest…"); clear(result); result.append(panel("Import validation", [details("Canonical ingestion plan", data)])); }
  async function registerImport(importId) { const task = await action(() => api(`/imports/${encodeURIComponent(importId)}/register`, { method: "POST", body: {} }), "Registering through canonical ingestion…"); if (task) location.hash = "#/tasks"; }
  app.append(pageHead("Dataset downloads", "Navigation never starts a download. Every allowed sample requires a review and confirmation."), panel("Canonical sources", [sources.length ? table(["Source", "License", "Local state", "Sample", "Reason", "Actions"], sources.map((source) => [source.name || source.source_id, badge(source.license_state), badge(source.local_state), source.estimated_sample_bytes, source.download?.reason, sourceActions(source)])) : node("div", { class: "state", text: "No canonical source records are available." })]), result, panel("Managed imports", [imports.length ? table(["Import", "Manifest", "Actions"], imports.map((item) => [item.import_id, badge(item.manifest_present ? "AVAILABLE" : "BLOCKED"), node("div", { class: "toolbar compact" }, [button("Validate", () => validateImport(item.import_id), { class: "secondary", disabled: !item.manifest_present }), button("Register", () => registerImport(item.import_id), { disabled: !item.manifest_present })])])) : node("div", { class: "state", text: "No managed imports are staged under data/imports." })]), panel("Recent downloads", [downloads.length ? table(["Task", "Source", "Status", "Phase", "Progress"], downloads.map((item) => [node("a", { href: "#/tasks", text: item.task_id }), item.target_id, badge(item.status), item.phase, `${Math.round((item.progress || 0) * 100)}%`])) : node("div", { class: "state", text: "No dataset download has been requested." })]));
}

async function renderTasks() {
  const data = await api("/tasks"); const tasks = data.tasks || []; app.append(pageHead("Operation tasks", "Persistent task records survive local service restarts.", [button("Refresh Tasks", route, { class: "secondary" })]));
  if (!tasks.length) return app.append(node("div", { class: "state", text: "No operation tasks are recorded." }));
  async function cancelTask(taskId) { await action(() => api(`/tasks/${encodeURIComponent(taskId)}/cancel`, { method: "POST", body: {} }), "Requesting cooperative cancellation…"); await route(); }
  app.append(table(["Task", "Operation", "Target", "Status", "Phase", "Progress", "Detail", "Action"], tasks.map((item) => [item.task_id, item.kind, item.target_id, badge(item.status), item.phase, `${Math.round((item.progress || 0) * 100)}%`, item.error?.message || item.detail || "—", button("Cancel", () => cancelTask(item.task_id), { class: "secondary", disabled: !["QUEUED", "RUNNING"].includes(item.status), title: ["QUEUED", "RUNNING"].includes(item.status) ? "Request cooperative cancellation" : "Task is no longer cancellable" })])));
  if (tasks.some((item) => ["QUEUED", "RUNNING"].includes(item.status))) state.taskPoll = setTimeout(() => { if (location.hash.startsWith("#/tasks")) route(); }, 2000);
}
...
async function renderPreflight() {
  ...
  async function run() { const data = await action(() => api("/preflight", { method: "POST", body: { plan_id: select.value, write_probe: false } }), "Running canonical preflight without a write probe…"); const plan = await api(`/plans/${encodeURIComponent(select.value)}`); const capability = plan.execution_capability || { available: false, reason: "Run preflight to determine execution capability" }; clear(result); result.append(panel("Preflight result", [kv({ "Final status": badge(data.final_status), "Ready": data.ready ? "YES" : "NO", "Adapter identity": data.adapter_identity, "Dataset identity": data.dataset_identity }), ...(data.sections || []).map((section) => panel(section.name, [badge(section.status), table(["Check", "Status", "Detail", "Blocker"], (section.checks || []).map((check) => [check.name, badge(check.status), check.detail, check.blocker ? "YES" : "NO"]))])), button("Start Real Training", async () => { const task = await action(() => api(`/training/${encodeURIComponent(select.value)}/start`, { method: "POST", body: {} }), "Re-running preflight and starting canonical training…"); if (task) location.hash = "#/tasks"; }, { disabled: !capability.available, title: capability.reason || "Start canonical training" }), node("p", { class: "muted", text: capability.reason || "All canonical execution gates passed." }), details("Canonical report", data)])); }
...
async function renderModelCatalog(parts) {
  if (parts[1]) return renderModelCatalogDetail(parts[1]); const models = (await api("/model-catalog")).models || []; app.append(pageHead("Published model catalog", "Eight evidence-backed entries; local actions exist only where canonical adapters exist."));
  app.append(table(["Model", "Benchmark", "Training", "Assets", "GPU", "Download"], models.map((item) => [node("a", { href: `#/model-catalog/${encodeURIComponent(item.catalog_id)}`, text: item.name }), badge(item.benchmark?.status), badge(item.training_supported ? "AVAILABLE" : "UNAVAILABLE"), badge(item.assets?.status), item.gpu_required ? "REQUIRED" : "NOT DECLARED", badge(item.download?.available ? "AVAILABLE" : "BLOCKED")])));
  app.append(node("div", { class: "callout warn", text: "No approved model download manifest is registered. Clouda Lab will not fetch model weights." }));
}

async function renderModelCatalogDetail(catalogId) {
  const encoded = encodeURIComponent(catalogId); const model = await api(`/model-catalog/${encoded}`); const assetId = node("input", { placeholder: "Managed asset ID", "aria-label": "Managed asset ID" }); const result = node("div");
  async function configure() { await action(() => api(`/model-catalog/${encoded}/assets`, { method: "POST", body: { asset_id: assetId.value } }), "Configuring a server-managed model directory…"); await route(); }
  async function verify() { const data = await action(() => api(`/model-catalog/${encoded}/verify`, { method: "POST", body: {} }), "Starting canonical local model verification…"); clear(result); result.append(panel("Verification task", [kv({ "Task": data.task_id, "Status": badge(data.status) }), node("a", { href: "#/tasks", text: "Open Tasks" })])); }
  async function remove() { const plan = await action(() => api(`/model-catalog/${encoded}/removal-plan`, { method: "POST", body: {} }), "Preparing recoverable removal…"); const task = await confirmPlan(plan, "/model-removals", "Remove managed model assets to recoverable trash"); if (task) location.hash = "#/tasks"; }
  app.append(pageHead(model.name, model.model_identifier, [node("a", { href: "#/model-catalog", text: "Back to catalog" })]), node("div", { class: "grid two-col" }, [panel("Capabilities", [kv({ "Benchmark": badge(model.benchmark?.status), "Training": badge(model.training_supported ? "AVAILABLE" : "UNAVAILABLE"), "Adapter": model.training_adapter, "GPU required": model.gpu_required ? "YES" : "NO", "Local inference": badge(model.local_inference_supported ? "AVAILABLE" : "UNAVAILABLE") })]), panel("Assets", [kv({ "Status": badge(model.assets?.status), "Managed ID": model.assets?.asset_id, "Location": model.assets?.location, "Last validation": model.assets?.last_validation })])]), panel("Managed actions", [node("div", { class: "toolbar" }, [assetId, button("Configure Assets", configure, { disabled: !model.training_supported, title: model.training_supported ? "Use an existing directory under data/models" : "No canonical local adapter is registered" }), button("Verify Assets", verify, { disabled: !model.training_supported || !model.assets?.asset_id, title: !model.assets?.asset_id ? "Configure a managed asset ID first" : "Run canonical verification" }), button("Remove Assets", remove, { class: "secondary", disabled: !model.assets?.asset_id, title: !model.assets?.asset_id ? "No managed assets are configured" : "Moves assets to recoverable trash" })]), node("p", { class: "muted", text: model.download?.reason }), details("Dependency remediation (display only)", model.dependencies?.remediation)]), result);
}
...
async function renderRunDetail(runId) {
  ...
  const resumeResult = node("div"); async function checkResume() { const data = await action(() => api(`/runs/${encoded}/resume-check`, { method: "POST", body: {} }), "Validating canonical checkpoint compatibility…"); clear(resumeResult); resumeResult.append(panel(data.compatible ? "Resume Compatibility Passed" : "Resume Blocked", [kv({ "Execution": badge(data.status), "Checkpoint": data.checkpoint?.checkpoint_id || data.checkpoint?.path, "Integrity": badge(data.integrity), "Configuration Match": badge(data.configuration_match), "Dataset Match": badge(data.dataset_match), "Model Match": badge(data.model_match), "Reason": data.reason || data.execution_reason }), button("Resume Training", async () => { const task = await action(() => api(`/runs/${encoded}/resume`, { method: "POST", body: {} }), "Starting canonical checkpoint resume…"); if (task) location.hash = "#/tasks"; }, { disabled: !data.available, title: data.execution_reason || data.reason || "Resume from verified checkpoint" })])); }
  app.append(panel("Resume", [button("Validate Resume", checkResume, { class: "secondary" }), node("p", { class: "muted", text: "Resume is enabled only after canonical checkpoint integrity and identity checks pass." }), resumeResult]));
}
...
async function renderBenchmarkWorkspace() {
  const [workspace, resultData] = await Promise.all([api("/benchmark-workspace"), api("/benchmark-results")]); const models = workspace.models || []; const selection = node("div", { class: "check-grid" }); const checks = [];
  models.forEach((model) => { const check = node("input", { type: "checkbox", value: model.catalog_id, id: `benchmark-${model.catalog_id}` }); checks.push(check); selection.append(node("label", { class: "check-item", for: check.id }, [check, node("span", { text: `${model.name} · ${model.benchmark?.status}` })])); }); const planResult = node("div");
  async function create() { const model_ids = checks.filter((item) => item.checked).map((item) => item.value); const data = await action(() => api("/benchmark-plans", { method: "POST", body: { model_ids } }), "Creating a deterministic benchmark plan…"); clear(planResult); planResult.append(panel("Benchmark plan", [kv({ "Plan": data.plan_id, "Benchmark": data.benchmark_id, "Execution": badge(data.execution?.available ? "AVAILABLE" : "BLOCKED"), "Reason": data.execution?.reason }), details("Eligibility", data.models)])); }
  const left = node("select"); const right = node("select"); models.forEach((model) => { left.append(node("option", { value: model.catalog_id, text: model.name })); right.append(node("option", { value: model.catalog_id, text: model.name })); }); if (models[1]) right.value = models[1].catalog_id; const comparison = node("div"); async function compare() { const data = await api(`/benchmark-comparison?left=${encodeURIComponent(left.value)}&right=${encodeURIComponent(right.value)}`); clear(comparison); comparison.append(panel("Actual result comparison", [kv({ "Winner": data.winner || "NOT RANKED", "Reason": data.reason }), details("Evidence", data)])); }
  app.append(pageHead("Benchmark workspace", "Protected evaluation data and published actual results; execution remains unavailable without a canonical runner."), node("div", { class: "grid two-col" }, [panel("Benchmark identity", [kv({ "Benchmark": workspace.benchmark?.benchmark_id, "Pages": workspace.benchmark?.distorted_page_count, "Primary metric": workspace.benchmark?.primary_metric, "Dataset purpose": badge(workspace.dataset?.purpose), "Training allowed": workspace.dataset?.training_allowed ? "YES" : "NO" })]), panel("Execution capability", [kv({ "Status": badge(workspace.execution?.available ? "AVAILABLE" : "BLOCKED"), "Reason": workspace.execution?.reason })])]), panel("Plan models", [selection, node("div", { class: "spacer" }), button("Create Benchmark Plan", create)]), planResult, panel("Compare published results", [node("div", { class: "toolbar" }, [left, right, button("Compare", compare, { class: "secondary" })]), comparison]), panel("Ranked actual results", [resultData.ranked?.length ? table(["Model", "Normalized CER", "CER", "WER", "Pages", "Hardware"], resultData.ranked.map((item) => [item.name, item.normalized_arabic_cer, item.cer, item.wer, item.pages, item.hardware])) : node("div", { class: "state", text: "No complete rankable result is published." })]));
}

async function renderStorage() {
  const data = await api("/storage"); app.append(pageHead("Managed storage", "Read-only accounting refuses symbolic links and never exposes private absolute paths."), node("div", { class: "grid cards" }, [card("Disk", badge(data.disk?.status), `${data.disk?.free_bytes || 0} bytes free`), card("Tasks", data.tasks?.total || 0, `${data.tasks?.pending || 0} pending`), card("Downloads", data.tasks?.downloads || 0, `${data.tasks?.pending_downloads || 0} pending`)]), panel("Categories", [table(["Category", "Location", "Bytes", "Present"], (data.categories || []).map((item) => [item.label, item.location, item.bytes, item.present ? "YES" : "NO"]))]), panel("Network policy", [kv({ "Default": badge(data.network_policy?.default), "Automatic downloads": data.network_policy?.automatic_downloads ? "ENABLED" : "DISABLED", "External providers": data.network_policy?.external_providers ? "ENABLED" : "DISABLED", "Explicit dataset samples": data.network_policy?.explicit_dataset_sample_downloads ? "AVAILABLE WITH CONFIRMATION" : "BLOCKED" })]));
}
